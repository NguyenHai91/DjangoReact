...
class UserManager(BaseUserManager):
  def create_user(self, full_name, email, phone, password=None, is_active=True, is_staff=False, is_admin=False):
    ...
    if re.fullmatch('^[a-z0-9.@#$%^&*-+~!]{4,}$', password.lower()) is None:
  ...
  def create_superuser(self, full_name, email, phone, password=None):
    user = self.create_user(full_name=full_name, email=email, phone=phone, password=password, is_staff=True, is_admin=True)
    return user
