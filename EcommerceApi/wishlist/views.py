...
class WishlistView(APIView):
  ...
  def post(self, request, *args, **kwargs):
    ...
    itemProduct = CartItem.objects.filter(wishlist=wishlist, product=product)
    if itemProduct.count() == 0:
      itemProduct = CartItem.objects.create(wishlist=wishlist, product=product)
      itemProduct.save()
  ...
  def delete(self, request, *args, **kwargs):
    ...
    wishlist, _ = Wishlist.objects.get_wishlist_or_create(request)
    item = CartItem.objects.filter(wishlist=wishlist, id=id_item).first()
    if item is not None:
      item.delete()
